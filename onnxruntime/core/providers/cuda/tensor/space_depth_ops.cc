...
static Status SpaceDepthOpCudaImpl(const cudaDeviceProp& prop,
                                   ...
                                   bool is_nhwc) {
  if (is_nhwc) {
    TensorShape virtual_input_shape{batch_size, in_dim2, in_dim3, in_dim4, in_dim5, in_dim1};
...
template<bool NHWC>
Status SpaceToDepth<NHWC>::ComputeInternal(OpKernelContext* context) const {
  ...
  if (NHWC) {
    output = context->Output(0, {batch, output_height, output_width, output_depth});
    virtual_output_shape = {batch, input_height / blocksize_, input_width / blocksize_,
                            blocksize_, blocksize_, input_depth};
    permutation = {0, 4, 3, 5, 1, 2};
  } else {
    output = context->Output(0, {batch, output_depth, output_height, output_width});
    virtual_output_shape = {batch, blocksize_, blocksize_, input_depth,
  ...
  ORT_RETURN_IF_ERROR(SpaceDepthOpCudaImpl(GetDeviceProp(), Stream(context), GetCublasHandle(context), input, *output, permutation, batch,
                                           ...
                                           virtual_output_shape, NHWC));

  return Status::OK();
}

template<bool NHWC>
Status DepthToSpace<NHWC>::ComputeInternal(OpKernelContext* context) const {
  const auto* tensor_pointer = context->Input<Tensor>(0);
  if (tensor_pointer == nullptr) return Status(common::ONNXRUNTIME, common::FAIL, "input count mismatch");
  const Tensor& input = *tensor_pointer;
  ...
  if (NHWC) {
    ...
    output = context->Output(0, {batch, output_depth, output_height, output_width});
    virtual_output_shape = {batch, input_depth / blocksize_ / blocksize_,
                                   input_height, blocksize_, input_width, blocksize_};
  }
